...
tab_model(model3, model2, model)
plot_models(model3, model2, model)


model <- glm(vs ~ gear, family = binomial, data=mtcars)
model2 <- glm(vs ~ gear+mpg, family = binomial, data=mtcars)

tab_model(model, model2)
plot_models(model, model2)

tab_xtab(mtcars$vs, mtcars$carb)

table1(~mpg+wt+cyl+carb|vs*am, data=mtcars)
...
# Link to material on gt/gtsummary
